...
var contains = function(nameArr, name, cb){

return nameArr.includes(name) ? cb(true) : cb(false);
...
// nameArr = ['Tyler', 'Cahlan', 'Ryan', 'Colt', 'Tyler', 'Blaine', 'Cahlan'];

function uniq(nameArr, cb) {
  var result=[nameArr[0]];
  for (var i = 1; i < nameArr.length;i++) {
    if (!result.includes(nameArr[i])) {
...
function each(nameArr, cb){
  ...
  nameArr.forEach(function(curr,i) {
    return cb(curr,i);
  })
...
// and returns that user.
 ...
 function getUserById(userthing,id,cb) {

   userthing.forEach(function(curr) {
     if (curr.id == id) {
       cb(curr);
     }
   })
